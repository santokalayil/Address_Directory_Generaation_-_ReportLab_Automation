...
def process():
    ...
    def update_families_sqlite_table(fam_id):
        print(f"UPDATING {fam_id}")
        table_name = "families"
        # family = families.loc[families['FamID'] == fam_id].T.iloc[:, 0]
        # print(families.loc[families['FamID'] == fam_id].reset_index().T.iloc[:, 0])
        # import sys
        # sys.exit()
    ...
    def update_cur_addr_sqlite_table(fam_id):
        ...
        # added problem
        house_building_name = family[
            'House Name / Building Name']  # if family['House Name / Building Name'] != '"REVA"' \
    ...
    def insert_new_records_sqlite_table():
        new_records_text = os.path.join('database', 'new_records.txt')
        with open(new_records_text, 'r') as txt:
            new_records = [int(i) for i in txt.read().split(',')]

        sqlite_db_path = os.path.join('database', 'sqlite.db')
        for fam_id in new_records:
            print(f"Updating new record of {fam_id}")
            family = families.loc[families['FamID'] == fam_id].reset_index().T.iloc[:, 0]

            # inserting into families
            qry = f'''
                INSERT
                INTO
                families('famid', 'email', 'where')
                VALUES({fam_id}, '{family['Email Address']}', '{family['Locality']}');
            '''
            post_query(qry, sqlite_db_path)

            # inserting into cur_addr
            qry = f'''
                INSERT
                INTO
                cur_addr('famid', 'house_flat_no', 'house_building_name', 'street', 'locality','city', 'pin')
                VALUES({fam_id}, "{family['House Number  / Flat Number']}", "{family['House Name / Building Name']}",'{family['Street']}','{family['Locality']}','{family['City']}','{family['Pin Code']}');
            '''
            post_query(qry, sqlite_db_path)

            # inserting into nat_addr
            qry = f'''
                INSERT
                INTO
                nat_addr('famid', 'house_name', 'po', 'place', 'district','state', 'pin')
                VALUES({fam_id}, "{family['House Name']}", "{family['Post Office']}",'{family['Place']}','{family['District']}','{family['State']}','{family['Pin Code.1']}');
            '''
            post_query(qry, sqlite_db_path)

            # inserting into nat_parish
            qry = f'''
                INSERT
                INTO
                nat_parish('famid', 'name', 'place', 'diocese')
                VALUES({fam_id}, "{family['Name of Native Parish']}", "{family['Place of Native Parish']}",'{family['Diocese of the Native Parish']}');
            '''
            post_query(qry, sqlite_db_path)

        print(f"Inserted new records to tables")

    insert_new_records_sqlite_table()

    def insert_new_members_into_sqlite_table():
        new_records_text = os.path.join('database', 'new_records.txt')
        with open(new_records_text, 'r') as txt:
            new_records = [int(i) for i in txt.read().split(',')]
        table_name = 'members'
        ...
        new_members = list(chain(*[list(members[members.famid == rec].index) for rec in new_records]))
        for i in new_members:
            ...
            qry = f''' 
                INSERT
                INTO
                {table_name}('famid', 'member_name', 'rltshp', 'prof', 'phone', 'dob', 'dom', 'blood_group')
                VALUES({famid}, "{member_name}", "{rltshp}",'{prof}','{phone}','{dob}','{dom}','{blood_group}');
            '''
            sqlite_db_path = os.path.join('database', 'sqlite.db')
            post_query(qry, sqlite_db_path)
        print('Completed insertion of new Records into members table')

    insert_new_members_into_sqlite_table()
